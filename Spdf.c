...
#define PORT 8124    // Port number for Spdf server
...
void handle_ufile(int smain_socket, char *filename, char *destination_path)
{
    ...
    snprintf(full_path, sizeof(full_path), "%s/%s/%s", getenv("HOME"), "spdf", destination_path);
    ...
    fclose(dest);
    send_message_to_smain(smain_socket, COLOR_GREEN "File uploaded successfully to Spdf\n" COLOR_RESET);
}
...
{
    char full_path[BUFSIZE];
    snprintf(full_path, sizeof(full_path), "%s/spdf/%s", getenv("HOME"), filename);
    ...
    send_message_to_smain(smain_socket, COLOR_GREEN "File transfer complete from Spdf\n" COLOR_RESET);
}
...
void handle_rmfile(int smain_socket, char *filename)
{
    ...
    {
        send_message_to_smain(smain_socket, COLOR_GREEN "File removed successfully from Spdf\n" COLOR_RESET);
    }
    else
    {
...
// Function to create a tarball of all .pdf files
void handle_dtar(int smain_socket)
{
    char tar_filename[BUFSIZE];
    snprintf(tar_filename, sizeof(tar_filename), "%s/pdffiles.tar", getenv("HOME"));

    char command[BUFSIZE];
    snprintf(command, sizeof(command), "tar -cvf %s -C %s/spdf .", tar_filename, getenv("HOME"));
    if (system(command) != 0)
    {
        send_message_to_smain(smain_socket, COLOR_RED "Error: Tarball creation failed in Spdf\n" COLOR_RESET);
        return;
    }
    ...
    if (!tar_file)
    {
        ...
        send_message_to_smain(smain_socket, COLOR_RED "Error: Failed to open tar file in Spdf\n" COLOR_RESET);
        return;
    }
    ...
    fclose(tar_file);
    send_message_to_smain(smain_socket, COLOR_GREEN "Tar file transfer complete from Spdf\n" COLOR_RESET);
...
int main()
{
    ...
    printf(COLOR_GREEN "Spdf server listening on port %d\n" COLOR_RESET, PORT);
